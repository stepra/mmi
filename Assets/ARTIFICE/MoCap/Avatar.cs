...
public class Limb
{
	...
	public string spawnParentObjectName="TrackingCamera";
	...
	public void StartTrackingLimb()
	{
		...
		//Creates an avatarObject we can append the TrackBone Objects on
		if(!avatarObj)
		{
		...
		{
			Console.WriteLine("Error: Limb::StartTrackingLimb: gO null");
		}
		initialized=true;
	}
	/// <summary>
...
public class Avatar : MonoBehaviour {
	...
	void Start () 
	{
    	...
    	{
			l.StartTrackingLimb();//create a TrackBone script for each en that has a transform attached to it
		}
		initialized=true;
	...
	void Update () 
	{
		...
		if((!nV)||(nV.isMine))
		{
			...
			{
				translationObject.localPosition=limbArray[3].getTrackedTransform().localPosition;
			}
		}			
	}
	void OnEnable()
